...
@register.simple_tag(name='is_production', takes_context=True)
def is_production(context):
  try:
    req = context["request"]
    full_path = ('http', ('', 's')[req.is_secure()], '://', req.META['HTTP_HOST'])
  except:
    full_path = 'www.device42.com'
  return 'www.device42.com' in ''.join(full_path)
